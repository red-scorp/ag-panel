...
public:
  explicit AbstractFixedFont():
    ...
    m_GlyphData(nullptr),
    m_DataRangeBegin(0),
    m_DataRangeEnd(0) { Init(); }
  virtual ~AbstractFixedFont() { Exit(); }

  /*! Get normal X size of font glyphs in pixels */
  virtual uint8_t GetGlyphXSize(void) const { return m_GlyphXSize; };
  /*! Get normal Y size of font glyphs in pixels */
  virtual uint8_t GetGlyphYSize(void) const { return m_GlyphYSize; };

  virtual const Glyph *GetGlyph(uint8_t number) { /* TODO: do a magic here! */ };
