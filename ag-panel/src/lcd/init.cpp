 ...
 */
AbstractLCD *initLCD(
    AbstractBacklight *p_Backlight,   /**< Pointer to configured backlight class */
    AbstractFont *p_Font              /**< Pointer to configured font class */
) {
    AbstractLCD *p_LCD = nullptr;

    #if defined(LCD_TEXT_4BIT) || defined(LCD_TEXT_8BIT)
        #if defined(LCD_TEXT_4BIT)
            p_LCD = new PPITextLCD(p_Backlight, LCD_COLS, LCD_ROWS,
        ...
        p_LCD = new I2CAIP31068TextLCD(p_Backlight, LCD_COLS, LCD_ROWS, LCD_I2C_ADDR);
    #elif defined(LCD_TEXT_SPI_AIP31068)
        p_LCD = new SPIAIP31068TextLCD(p_Backlight, LCD_COLS, LCD_ROWS, LCD_PIN_SS, LCD_PIN_SCLK, LCD_PIN_MOSI, LCD_PIN_MISO);
